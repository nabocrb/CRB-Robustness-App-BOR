...
prep_satisficing=data.frame(metrics_4app$satisficing, add_to_tradeoff)
...
output$pcSatisficing=renderPlotly({
  ...
  p=par_coords(data=data.frame(Explore_all$df, global.filter=in_global_filter$y), n_var=8, 
             color_var = input$colorExplore, title='Satisficing-related metrics', labels=labelsSatisficing, max_cols = 2:4, source='satisficing', policy_ID = tableSelectExplore$constraints, colorbarTitle = input$colorExplore)
  p=event_register(p,"plotly_restyle")
  p
