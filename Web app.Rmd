...
KenStone.samples=read.table("KenStone.samples.txt", header = T)
...
observeEvent(input$KSApply,{ # Get KS index when user hits apply button in settings page
  ...
  if (input$KSmetric=="Decision Variables") { # get policy ID from KenStone.samples
    KS$index=KenStone.samples$DV[1:input$KSnSample] 
  } else { # baseline performance
    KS$index=KenStone.samples$baseline[1:input$KSnSample]
  }
}, ignoreInit = F, ignoreNULL = F)

observeEvent(input$KSApply, { # Subset prep dataframes. Execute on apply button in settings page
...
observeEvent(input$HurwicCalc, {
  ...
  metrics.list$l$Hurwicz.OP=temp2 # update both metrics list
  metrics_4app$l$Hurwicz.OP=temp2
...
sliderInput("KSnSample", label = "Number of policies", min = 2, max=463, value=463, round=2)
...
# cat(file=stderr(), 'dimensions:', check, "\n") # prints dimension to R console
    ...
    temp2=dplyr::filter(temp, policy %in% KS$index) # subset to KS index
    metrics.list$l$satisficing=temp2 # update both metrics list
    metrics_4app$l$satisficing=temp2
