...
# update satisficing if user changes thresholds or objectives

observeEvent(satisdf$updateTrigger, { 
  
    prep$satisficing=data.frame(satisdf$df, add_to_tradeoff)
...
output$pc1=renderPlotly({
  ...
  if (input$KeepPoliciesTF==TRUE){
    data=data.frame(baseline$all,global.filter=in_global_filter$y)
...
observeEvent(satisdf$updateTrigger, { # this observer updates the satisficing df when user recalculates
